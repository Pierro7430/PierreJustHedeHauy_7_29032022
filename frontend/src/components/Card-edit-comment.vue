<template>
    <div class="create-comment">
        ...
        <div class="create-comment_form">
            ...
            <div class="form-row">
                <p>Description</p>
                <textarea v-model="comment.description" v-on:input="checkFormIsValid()" class="form-row_input" rows="10"/>
            </div>

            <div @click="checkFormIsValid()" class="form-img">
                <p>Image</p>
                ...
                <div v-if="!comment.image && !comment.urlImg" class="form-img_upload">
                    <input id="file" v-on:input="checkFormIsValid()" @change="onFileChange" type="file" />
                    <label for="file">
        ...
        <div class="create-comment_btn">
            ...
            <button @click="updateComment()" :disabled="!this.editFormValidated" class="btn-basic btn-basic--submit-A" :class="{'btn-basic--disabled' : !this.editFormValidated}">Modifier</button>
        </div>
...
export default {
    ...
    methods: {
        ...
        // Vérification si il y a du contenu (image ou txt)
        checkFormIsValid: function () {
            if(this.comment.urlImg || this.comment.image || this.comment.description){
                this.editFormValidated = true;
            }
            else {
                this.editFormValidated = false;
            }
        },

        // Changement ou ajout d'image
        onFileChange(event) {
        ...
        // Envoie de commmentaire update dans le backend
        ...
        cancelComment: function () {
            ...
            this.$router.go("/comment/:" + this.comment.commentId)  
        },
    },
}
